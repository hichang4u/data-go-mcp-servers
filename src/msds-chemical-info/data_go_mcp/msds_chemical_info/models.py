...
class ChemicalListItem(BaseModel):
    ...
    last_date: Optional[str] = Field(default=None, alias="lastDate", description="최종 갱신일")
    kosha_confirm: Optional[str] = Field(
        default=None, alias="koshaConfirm", description="KOSHA 확인"
    )
    open_yn: Optional[str] = Field(default=None, alias="openYn", description="공개여부")
...
class MsdsSection(BaseModel):
    ...
    error: Optional[str] = Field(default=None, description="조회 실패 시 오류 메시지")
    ...
    def get_formatted_content(self) -> str:
        ...
        for item in self.items:
            ...
            if item.msds_item_no:
                lines.append(
                    f"{indent}{item.msds_item_no}. {item.msds_item_name_kor}: {item.item_detail or '자료없음'}"
                )
            else:
                if item.lev == 1:
                    lines.append(
                        f"{indent}• {item.msds_item_name_kor}: {item.item_detail or '자료없음'}"
                    )
                else:
                    lines.append(
                        f"{indent}- {item.msds_item_name_kor}: {item.item_detail or '자료없음'}"
