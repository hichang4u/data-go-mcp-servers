...
async def get_bid_detail(
    ...
    """
    async with tool_errors():
        start_dt, end_dt = _range(start_date, end_date, default_days=30)
        async with PpsNarajangteoAPIClient() as client:
            for page_no in range(1, 4):
                ...
                if len(items) < 999:
                    break
        raise ToolError(
            f"입찰공고번호 {bid_notice_no}를 {start_dt[:8]} ~ {end_dt[:8]} 범위에서 찾을 수 없습니다. "
            "공고일을 알면 start_date/end_date 로 범위를 지정하세요."
        )
