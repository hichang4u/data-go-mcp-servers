...
def collect_project_info() -> Dict[str, Any]:
    ...
    # API Name
    while True:
        api_name = prompt_with_default("API name (kebab-case, e.g., 'weather-forecast')").lower()
    ...
    info["api_display_name"] = prompt_with_default("Display name", default_display)

    # Korean name
    info["api_korean_name"] = prompt_with_default("Korean name (한국어 이름)")

    # Description
    info["api_description"] = prompt_with_default("Description")
    ...
    info["api_key_env_name"] = prompt_with_default(
        "Environment variable name for API key", default_env
    )

    # GitHub username
    info["github_username"] = prompt_with_default("GitHub username", "yourusername")

    # Python version
    info["python_version"] = prompt_with_default("Minimum Python version", "3.10")

    # Version
    info["version"] = prompt_with_default("Initial version", "0.1.0")

    # Author
    info["author_name"] = prompt_with_default("Author name", "Data.go.kr MCP Servers Contributors")
...
def main():
    ...
    if create_project(info):
        ...
        print("   2. Edit api_client.py to add your API methods")
        print("   3. Update server.py with MCP tools")
        print(f"   4. Set your API key: export {info['api_key_env_name']}='your-key'")
        print("   5. Test: uv run pytest tests/")
