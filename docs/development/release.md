...
- 각 `src/*/pyproject.toml` 의 `version`. 서버는 각자 올린다 — 툴이 바뀐 서버만 bump (v0.5.0 시점: nps·fsc 0.5.0, dart 0.1.0, 나머지 0.3.0), core 는 독립 (0.2.x).
- fsc 의 `SERVER_VERSION` 은 패키지 메타데이터에서 읽으므로 손댈 곳이 없다.
- git 태그 `vX.Y.Z` 는 저장소 전체 스냅샷이며 서버 버전과 무관하게 올라간다. 사용자는 `data-go-mcp-servers@v0.5.0#subdirectory=…` 로 고정할 수 있다.
...
4. `uv lock` 후 커밋: `chore(release): v0.5.0`.
5. 태그와 릴리스:
   ```bash
   git tag -a v0.5.0 -m "v0.5.0"
   git push origin main --tags
   gh release create v0.5.0 --title "v0.5.0" --notes-file <notes>
   ```
6. 새 환경에서 확인:
   ```bash
